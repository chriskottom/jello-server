...
class V3::BoardSerializerTest < ActiveSupport::TestCase
  ...
  test 'includes links for the Board' do
    ...
    self_link = { rel: :self, href: v3_board_url(@board) }
    assert_link self_link, serialized_board

    creator_link = { rel: :creator, href: v3_user_url(@board.creator) }
  ...
  def serialized_board
    serialized_data(@board, V3::BoardSerializer)
  end
end
